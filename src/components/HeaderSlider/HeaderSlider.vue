...
function changeCounter() {
  const randomValue = Math.random() > 0.45 ? 1 : -1; // Случайное значение 1 или -1
...
onMounted(() => {
  setInterval(changeCounter, getRandomTime()); // Запускать функцию changeCounter через случайное время
});
